...
- **Host kicked to `/game` with “Unknown or missing room” after “loading from episode pack”** — The Node process was killed (often OOM). Game rooms live only in memory, so a restart looks like a missing room. Current builds stream the j-archive pack and cache one decade at a time to stay under free-tier RAM; redeploy that fix. Prefer a paid instance or a persistent disk for `CLUES_DB_PATH` / `JARCHIVE_CACHE_DIR` so 2020s imports and pack caches survive restarts.
